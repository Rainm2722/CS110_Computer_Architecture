...
v_add_optimized_adjacent (double *x, double *y, double *z)
{
  #pragma omp parallel
  {
    ...
    for (int i = 0; i < ARRAY_SIZE; i++){
      if (i % omp_get_num_threads() == thread_ID){
          z[i] = x[i] + y[i];
      }
    }
  }
...
v_add_optimized_chunks (double *x, double *y, double *z)
{
  #pragma omp parallel
  {
    int workload = ARRAY_SIZE / omp_get_num_threads();
    int end = workload * (omp_get_thread_num() + 1);
    if ( omp_get_thread_num() == omp_get_num_threads() - 1 ){
      end = ARRAY_SIZE; 
    }
    for (int i = workload * omp_get_thread_num(); i < end; i++){
      z[i] = x[i] + y[i];
    }
  }
}
